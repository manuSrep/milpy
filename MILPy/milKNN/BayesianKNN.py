...
class BayesianKNN(BaseEstimator):
    ...
    def fit(self, data):
        ...
        for i in range(0, self.n_train):
            ...
            for j in range(i + 1, self.n_train):
                if j < self.n_neg:
                    ...
                    bag_j = self.pos_train.get_B(
                        self.pos_train.keys[j - self.n_neg])[0]
        ...
        self.max_propab = np.zeros((self.k + 1,
        2))  # entries for k labels being in class 1 and 0 respectively

        # find order of nearest neighbours
        n_given_neg = np.argsort(self.dist_train[0:self.n_neg], axis=1)[:,
        0:self.k]
        n_given_pos = np.argsort(self.dist_train[self.n_neg:self.n_train],
            axis=1)[:, 0:self.k]
        ...
        self.max_propab[:, 0] /= np.sum(
            self.max_propab[:, 0])  # normalize for number of entries
        self.max_propab[:, 1] /= np.sum(
            self.max_propab[:, 1])  # normalize for number of entries
