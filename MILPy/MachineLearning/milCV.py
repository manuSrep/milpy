...
__all__ = ["KFold","train_test_split", "cross_val_predict", "cross_val_predictions", "cross_val_score", "validation_curve"]
...
#                 ('This GridSearchCV instance was initialized '
...
#                 self.scorer_, train, test, self.verbose, parameters,
...
def cross_val_predictions(estimator, data, cv, fit_params=None, n_jobs=1, pre_dispatch='2*n_jobs', method='predict'):
    ...
    if np.any(predictions_y is None):
        predictions_y = None
    test_keys = [key for _, _, key in prediction_blocks]
...
def _fit_and_score(estimator, data, scorer, train, test, parameters, fit_params, return_train_score=False):
    """
